...
app.put('/api/productos/listar/:id', (req, res) => {
  const findId = req.params.id;
  const response = products.getProductId(findId)
  if (!response) {
      return res.status(400).json({
  ...
  const posicion = products.map((aProduct) => aProduct.id).indexOf(findId);
  const title = req.body.title;
  const price = req.body.price;
  const thumbnail = req.body.thumbnail;
  if (!title || !price || !thumbnail) {
  ...
  products[posicion].title = title;
  products[posicion].price = price;
  products[posicion].thumbnail = thumbnail;

  res.json(products[posicion]);
});